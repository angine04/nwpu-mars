...
# from misc.log import log
# from misc.bbox import makeAnchors
# from model.base.backbone import Backbone  # 原始Backbone
# from model.base.swin_backbone import SwinBackbone  # 新增的SwinBackbone
# from model.base.neck import Neck
# from model.base.head import DetectHead
...
# class YoloModel(nn.Module):
#     @classmethod
#     def loadModelFromFile(cls, mcfg, modelFile, use_swin=False):
#         if modelFile is None:
#             raise ValueError("Model file not set")
#         if not os.path.exists(modelFile):
#             raise ValueError("Model file not exists")
#         model = cls(mcfg, use_swin=use_swin)
#         model.load(modelFile)
#         return model

#     def __init__(self, mcfg, use_swin=False):
#         super(YoloModel, self).__init__()

#         self.mcfg = mcfg
#         self.inferenceMode = False
#         self.use_swin = use_swin  # 是否使用Swin Backbone的标志
...
#             self.backbone = SwinBackbone(w, r, n, img_size=mcfg.inputShape[0])
#         else:
#             log.inf("Using Default Backbone")
#             self.backbone = Backbone(w, r, n)
...
#             zo: (B, nc + regMax * 4, 20, 20)
#         """
#         # SwinBackbone和原始Backbone的输出格式保持一致，无需修改后续处理
#         _, feat1, feat2, feat3 = self.backbone.forward(x)
#         _, X, Y, Z = self.neck.forward(feat1, feat2, feat3)
#         xo, yo, zo = self.head.forward(X, Y, Z)
#         return xo, yo, zo
...
#         state_dict['_use_swin'] = torch.tensor([1] if self.use_swin else [0])
#         torch.save(state_dict, modelFile)
#         if verbose:
#             log.inf("Yolo model state saved at {}".format(modelFile))
...
#             # Swin预训练权重可能需要特殊处理
#             new_state_dict = {}
#             for k, v in pretrainedState.items():
#                 if k.startswith('features.'):
#                     new_k = k.replace('features.', '')
#                     new_state_dict[new_k] = v
#             pretrainedState = new_state_dict
...
#         # 检查是否包含backbone类型信息
#         if '_use_swin' in modelState:
#             use_swin = bool(modelState['_use_swin'].item())
#             if use_swin != self.use_swin:
#                 log.red(f"Model type mismatch! Loaded model uses {'Swin' if use_swin else 'Default'} backbone, but current model uses {'Swin' if self.use_swin else 'Default'} backbone")
#                 raise ValueError("Backbone type mismatch during loading")
#             del modelState['_use_swin']
